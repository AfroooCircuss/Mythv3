...
public class VClipDown extends Module {
    ...
    public SettingNumber downValue = new SettingNumber(5 , 1, 10, 1, "Value");
    public SettingBool mode = new SettingBool(false, "Smart");
    public SettingNumber startValue = new SettingNumber(1 , 1, 9, 1, "Start Value");

    public VClipDown()
    {
        super(ModuleType.MISC, "VClip Down", Keyboard.KEY_DOWN);
        this.abstractSettings.add(downValue);
        ...
        if(mode.value) {
            for (int i = (int) startValue.value; i < 11; i++) {
                ...
                if (Utility.getBlockState(belowBlockPos).N() instanceof fD && Utility.getBlockState(pos2).N() instanceof fD) {
                    ((DN) Utility.getThePlayer()).e(Utility.getPosX(), Utility.getPosY() - i - 1, Utility.getPosZ());
                    this.toggle();
                    return;
                }
            }
        } else {
            ((DN) Utility.getThePlayer()).e(Utility.getPosX(), Utility.getPosY() - downValue.value, Utility.getPosZ());
        }
        this.toggle();
    }
}
